...
public abstract class LRParser {
    ...
    public boolean accept(ArrayList<String> inputs) {
        ...
        while(index < inputs.size()){
            ...
            if(action == null){
                System.out.println("Action table entries at this state:" + actionTable[state]);
                return false;
...
//                System.out.println(leftSide);
    ...
    public String goToTableStr() {
        ...
        for (String variable : grammar.getVariables()) {
            str += String.format("%-15s",variable);
        }
        str += "\n";
        ...
        for (int i = 0; i < goToTable.length; i++) {
            ...
            for (String variable : grammar.getVariables()) {
                str += String.format("%15s",(goToTable[i].get(variable) == null ? "|" : goToTable[i].get(variable)+"|"));
            }
            str += "\n";
        }
        for (int j = 0; j < (grammar.getVariables().size()+1)*15+2; j++) {
